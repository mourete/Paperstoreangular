...
export class ProyectoComponent implements OnInit {
  ...
  constructor( private datePipe: DatePipe ,  public proyectoService:ProyectoService ,  public empresaService:EmpresaService ,   public marcaService:MarcaService ,     public config: DynamicDialogConfig , public ref: DynamicDialogRef) { }
  ...
  public getProyectoById ( proyectoId : number ){
    ...
    this.proyectoService.getProyectoById ( proyectoId, this.usuarioOID ).subscribe(
      (data)=>{
        ...
        if( this.proyecto.fechaIniText!=null ){
            this.proyecto.fechaIniDate= this.getFechaFromString ( this.proyecto.fechaIniText );
        } 

        if( this.proyecto.fechaFinText!=null ){
           this.proyecto.fechaFinDate = this.getFechaFromString( this.proyecto.fechaFinText );
        }
  ...
  public getRegionesByMarca(id: number ){
    ...
    if(id == 1 ) {
      ...
      marcaId = this.proyecto.marcaId;
  
    var regProyectos:ProyectoRegion[]=null;
    this.proyectoService.getProyectoRegionesByMarca (  this.proyecto.proyectoId  , this.selectedMarca.marcaId ,  this.usuarioSession.usuarioOID  ).subscribe(
      (data)=>{        
  ...
  public getDocumentos(){
    ...
    this.proyectoService.getDocumentosByProyecto   (  this.proyecto.proyectoId  ,   this.usuarioSession.usuarioOID  ).subscribe(
      (data)=>{        
        docProyectos=data;
  ...
  public guadarProyecto(){
    ...
    if(  this.proyecto.clave ==null ||this.proyecto.clave=="" ){
      this.msgs.push({severity:'error', detail: "Se requiere capturar la clave del proyecto "  , summary:'Validation failed'});
      return;
    }

    if(  this.proyecto.nombre ==null ||this.proyecto.nombre =="" ){
      this.msgs.push({severity:'error', detail: "Se requiere capturar el nombre del proyecto "  , summary:'Validation failed'});
      return;
    }    
  

    if(  this.selectedEmpresa ==null   ){
      this.msgs.push({severity:'error', detail: "Se requiere seleccionar la empresa para el poryecto "  , summary:'Validation failed'});
      return;
    }   
    ...
    if( this.proyecto.fechaIniDate!=null  ){  
      let dateAsString =this.datePipe.transform( this.proyecto.fechaIniDate , 'dd/MM/yyyy');                
      this.proyecto.fechaIniText=dateAsString;
      
    } 

    if( this.proyecto.fechaFinDate!=null  ){  
      let dateAsString =this.datePipe.transform( this.proyecto.fechaFinDate , 'dd/MM/yyyy');                
      this.proyecto.fechaFinText=dateAsString;      
    } 


    
   if(  this.proyectoRegionesSelected !=null && this.proyectoRegionesSelected.length>0 ){
   ...
   this.setDocumentosSelected();
    ...
    this.proyectoService.saveProyecto (  this.proyecto, this.usuarioOID ).subscribe(
      (data)=>{
        this.proyecto=data;     
  ...
  public setDocumentosSelected():string{
    ...
    if(  this.proyectoDocumentosSelected !=null && this.proyectoDocumentosSelected.length>0 ){
      
      for(  var i=0;i< this.proyectoDocumentosSelected.length ; i++ ){
          if(  documentosConcat==null ){
            documentosConcat=""+  this.proyectoDocumentosSelected[i].documentoId;
          }else{
            documentosConcat+="|"+  this.proyectoDocumentosSelected[i].documentoId;
          }
  ...
  public addRegion(){
   ...
   if( this.selectedProyectoRegionesLeft==null  || this.selectedProyectoRegionesLeft.length<=0 ){
      return
   }
   ...
   this.proyectoRegionesSelected=this.proyectoRegionesSelected.concat( this.selectedProyectoRegionesLeft  );
  ...
  public removeRegion(){
   ...
   if( this.selectedProyectoRegionesRight==null || this.selectedProyectoRegionesRight.length<=0  ){
      return
   }
  ...
  this.proyectoRegiones=this.proyectoRegiones.concat( this.selectedProyectoRegionesRight  );

  var  lstSelected=  this.selectedProyectoRegionesRight;
  this.proyectoRegionesSelected = this.proyectoRegionesSelected.filter(function(x   ) { 
    var proyReg:ProyectoRegion;
  ...
  public addDocumento(){
    ...
    if( this.proyectoDocumentos==null || this.proyectoDocumentos.length<=0 ){
      return;
    }

   if( this.selectedProyectoDocumentosLeft==null  || this.selectedProyectoDocumentosLeft.length<=0 ){
      return
   }
   ...
   this.proyectoDocumentosSelected=this.proyectoDocumentosSelected.concat( this.selectedProyectoDocumentosLeft  );
  ...
  public removeDocumento(){
   ...
   if( this.selectedProyectoDocumentosRight==null || this.selectedProyectoDocumentosRight.length<=0  ){
      return
   }
  ...
  this.proyectoDocumentos=this.proyectoDocumentos.concat( this.selectedProyectoDocumentosRight  );

  var  lstSelected=  this.selectedProyectoDocumentosRight;
  this.proyectoDocumentosSelected = this.proyectoDocumentosSelected.filter(function(x   ) { 
    var proyDoc:ProyectoDocumento;
