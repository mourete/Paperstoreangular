...
export class DisplayDocumentComponent implements OnInit {
  ...
  constructor( private documentService : DocumentService ,
               ...
               public dialogService: DialogService , private conceptoService : ConceptoService ,
  ...
  ngOnInit(): void {      
    ...
    this.usuarioOID=this.usuario.usuarioOID;
      ...
      this.items = [
        {label: 'Editar', icon: 'pi pi-pencil', command: (event) => {
           
            this.editarSeccion();
        }},
        {label: 'Eliminar sección', icon: 'pi pi-trash', command:(event) =>{
          this.confirmDeleteSeccion();
          }  
      
         },               
        {separator: true},
        {label: 'Agregar concepto', icon: 'pi pi-plus' , command:(event) =>{
            this.agregarConcepto(0);
        }  
        },
        {separator: true} ,
        {label: 'Agregar sección arriba', icon: 'pi pi-plus' , command:(event) =>{
          this.agregarSeccion(1);
        }
      },
      {label: 'Agregar sección abajo', icon: 'pi pi-plus' , command:(event) =>{
    ...
    this.itemsConcepto = [
      {label: 'Editar concepto', icon: 'pi pi-pencil', command: (event) => {         
          this.editarConcepto();
      }},
      {separator: true},
      {label: 'Eliminar concepto', icon: 'pi pi-trash', command:(event) =>{
        this.confirmDeleteConcepto();
       }    
      },               
      {separator: true},
      {label: 'Agregar concepto arriba', icon: 'pi pi-plus' , command:(event) =>{
          this.agregarConcepto(1);
        }
      },
      {label: 'Agregar concepto abajo', icon: 'pi pi-plus' , command:(event) =>{
  ...
  public agregarConcepto( dir: number ){
      ...
      if( dir==1 || dir==2 ){        
        ...
        if(  this.concepto!=null && this.seccion.conceptos!=null && this.seccion.conceptos.length>0 ){
 ...
 public addConceptoMem( concep: Concepto ){
  ...
  public clickBotonEditarConcepto(){
       ...
       if( this.seccion==null || this.seccion.conceptos==null || this.seccion.conceptos.length<=0  ){
           return;
       } 
...
public agregarSeccion(dir:number){
 ...
 public addSeccionToMem(sec : Seccion){
  ...
  public getDocumentForConfig(){
    ...
    this.disabled = true;
       this.documentService.getDocumentForConfig( this.documentoId , this.usuarioOID).subscribe( 
         (data)=>{
  ...
  public deleteConcepto() {  
    this.conceptoService.deleteConcepto ( this.concepto, this.usuarioOID ).subscribe((data)=>{
        console.log(data);
...
public deleteSeccion() {  
  this.seccionService.deleteSeccion ( this.seccion , this.usuarioOID).subscribe((data)=>{
      console.log(data);    
      ...
      if( seccionTmp!=null ){
        ...
        for(  var i=0;i< this.documento.secciones.length;i++ ){                     
          if(  this.documento.secciones[i].seccionOID== seccionTmp.seccionOID ){
...
public changeConceptoOrder(flagOrder:number){
  ...
  if( flagOrder==1 ){
   ...
   this.conceptoService.getConceptoByOID ( this.concepto.conceptoOID ,this.usuarioOID).subscribe( 
    (data)=>{
       this.concepto=data;
...
public getConceptosBySeccionOID( seccionOID:string ) {  
  this.conceptoService.getConceptoBySeccionOID ( seccionOID ,this.usuarioOID).subscribe((data)=>{
      console.log(data);        
...
public onMenuConceptoShow(event){
    ...
    if( this.concepto!=null  ){
      ...
      if( this.documento.secciones!=null && this.documento.secciones.length>0 ){
  
        for( var idx=0; idx< this.documento.secciones.length;  idx++  ){
          if(  this.concepto.seccionOID ==this.documento.secciones[idx].seccionOID ){
